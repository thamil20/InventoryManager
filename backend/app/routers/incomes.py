...
@router.post('/', response_model=schemas.incomes.Response)
...
@router.get('/', response_model=List[schemas.incomes.Response])
def get_all(skip: int = 0, limit: int = 50, db: Session = Depends(get_db)):
    return crud.incomes.get_all(db, skip=skip, limit=limit)

@router.get('/{income_id}', response_model=schemas.incomes.Response)
...
@router.get('/{income_type}', response_model=List[schemas.incomes.Response])
...
@router.put('/{income_id}', response_model=schemas.incomes.Response)
...
@router.delete('/{income_id}', response_model=schemas.incomes.Response)
